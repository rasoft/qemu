...
static void _dwc_ssi_gx_update_irq(DwcSsiStateGx *ssi)
{
    qemu_set_irq(ssi->irq, !!(ssi->risr.value & ssi->imr.value));
}

static void _dwc_ssi_gx_update_dma(DwcSsiStateGx *ssi)
{
    // TX
...
// Transmit
static void _dwc_ssi_gx_tx(DwcSsiStateGx *ssi)
{
    ...
    // Update TXEIR
    ssi->risr.txeir = 1;
    _dwc_ssi_gx_update_irq(ssi);

    // Update DMA
    _dwc_ssi_gx_update_dma(ssi);
}

// Receive
static void _dwc_ssi_gx_rx(DwcSsiStateGx *ssi)
{
    // Do Transfer
...
// TX and RX
static void _dwc_ssi_gx_enhance_xfer(DwcSsiStateGx *ssi)
{
...
static void _dwc_ssi_gx_normal_xfer(DwcSsiStateGx *ssi)
{
    // Flush data to bus
    _dwc_ssi_gx_tx(ssi);
...
// Register Read
static uint64_t _dwc_ssi_gx_read(void *opaque, hwaddr offset, unsigned size)
{
    DwcSsiStateGx *ssi = opaque;
    LOG("Read 0x%08X\n", (unsigned int)offset);
    ...
    // 0x404
    if (offset == DWC_SSIC_GX_REG_CS) {
        return ssi->chip_select;
    }
...
// Register Write
static void _dwc_ssi_gx_write(void *opaque, hwaddr offset, uint64_t value, unsigned size)
{
    DwcSsiStateGx *ssi = opaque;
    ...
    if (offset >= DWC_SSIC_REG_DR00 && offset <= DWC_SSIC_REG_DR35) {
        ...
        if (ssi->tx_count || fifo32_num_used(&ssi->tx_fifo) > ssi->txftlr.txfthr) {
            if (ssi->ctrlr0.spi_frf == 0) {
                _dwc_ssi_gx_normal_xfer(ssi);
            } else {
                _dwc_ssi_gx_enhance_xfer(ssi);
            }
        }
        return;
    }
...
static void _dwc_ssi_gx_reset(DeviceState *d)
{
    DwcSsiStateGx *ssi = DWC_SSI_GX(d);
...
static const MemoryRegionOps _dwc_ssi_gx_ops = {
    .read = _dwc_ssi_gx_read,
    .write = _dwc_ssi_gx_write,
    .endianness = DEVICE_NATIVE_ENDIAN,
};

static const VMStateDescription _vmstate_dwc_ssi = {
    .name = TYPE_DWC_SSI_GX,
    .version_id = 1,
    .fields = (VMStateField[]) {
...
static void _dwc_ssi_gx_realize(DeviceState *dev, Error **errp)
{
    SysBusDevice *sbd = SYS_BUS_DEVICE(dev);
    DwcSsiStateGx *s = DWC_SSI_GX(dev);

    memory_region_init_io(&s->iomem, OBJECT(s), &_dwc_ssi_gx_ops, s, "dwc-ssi", 0x1000);
...
static void _dwc_ssi_gx_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->realize = _dwc_ssi_gx_realize;
    dc->vmsd = &_vmstate_dwc_ssi;
    dc->reset = _dwc_ssi_gx_reset;
}

static const TypeInfo _dwc_ssi_gx_info = {
    .name          = TYPE_DWC_SSI_GX,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(DwcSsiStateGx),
    .class_init    = _dwc_ssi_gx_class_init,
};

static void _dwc_ssi_gx_register_types(void)
{
    type_register_static(&_dwc_ssi_gx_info);
}

type_init(_dwc_ssi_gx_register_types)
