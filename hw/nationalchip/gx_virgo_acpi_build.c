...
// #include "hw/nationalchip/virgo.h"
...
static void acpi_dsdt_add_cpus(Aml *scope, VirgoMachineState *vms)
{
...
static void acpi_dsdt_add_pci(Aml *scope, const MemMapEntry *memmap,
                              uint32_t irq, VirgoMachineState *vms)
{
    int ecam_id = VIRGO_ECAM_ID(vms->highmem_ecam);
    struct GPEXConfig cfg = {
        .mmio32 = memmap[VIRGO_PCIE_MMIO],
        .pio    = memmap[VIRGO_PCIE_PIO],
        .ecam   = memmap[ecam_id],
        .irq    = irq,
        .bus    = vms->bus,
    };
    ...
    if (vms->highmem_mmio) {
        cfg.mmio64 = memmap[VIRGO_HIGH_PCIE_MMIO];
    }
...
#ifdef CONFIG_TPM
static void acpi_dsdt_add_tpm(Aml *scope, VirgoMachineState *vms)
{
    PlatformBusDevice *pbus = PLATFORM_BUS_DEVICE(vms->platform_bus_dev);
    hwaddr pbus_base = vms->memmap[VIRGO_PLATFORM_BUS].base;
 ...
 */
static void
build_iort(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
    ...
    if (vms->iommu == VIRGO_IOMMU_SMMUV3) {
    ...
    if (vms->iommu == VIRGO_IOMMU_SMMUV3) {
        int irq =  vms->irqmap[VIRGO_SMMU] + ARM_SPI_BASE;
        ...
        /* Base address */
        build_append_int_noprefix(table_data, vms->memmap[VIRGO_SMMU].base, 8);
        /* Flags */
 ...
 */
static void
build_spcr(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
    ...
    build_append_int_noprefix(table_data,
                              vms->irqmap[VIRGO_UART] + ARM_SPI_BASE, 4);
 ...
 */
static void
build_srat(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
    int i;
    uint64_t mem_base;
    ...
    mem_base = vms->memmap[VIRGO_MEM].base;
 ...
 */
static void
build_gtdt(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
    ...
    build_append_int_noprefix(table_data, irqflags |
                              ...
                              4);
    /* Virtual timer GSIV */
    build_append_int_noprefix(table_data, ARCH_TIMER_VIRGO_IRQ + 16, 4);
    /* Virtual Timer Flags */
    build_append_int_noprefix(table_data, irqflags, 4);
...
static void
build_dbg2(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
...
static void
build_madt(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
    int i;
    VirgoMachineClass *vmc = VIRGO_MACHINE_GET_CLASS(vms);
    ...
    build_append_int_noprefix(table_data, memmap[VIRGO_GIC_DIST].base, 8);
    ...
    for (i = 0; i < MACHINE(vms)->smp.cpus; i++) {
        ...
        if (vms->gic_version == VIRGO_GIC_VERSION_2) {
            physical_base_address = memmap[VIRGO_GIC_CPU].base;
            gicv = memmap[VIRGO_GIC_VCPU].base;
            gich = memmap[VIRGO_GIC_HYP].base;
        }
    ...
    if (vms->gic_version != VIRGO_GIC_VERSION_2) {
        build_append_gicr(table_data, memmap[VIRGO_GIC_REDIST].base,
                                      memmap[VIRGO_GIC_REDIST].size);
        if (virgo_gicv3_redist_region_count(vms) == 2) {
            build_append_gicr(table_data, memmap[VIRGO_HIGH_GIC_REDIST2].base,
                                          memmap[VIRGO_HIGH_GIC_REDIST2].size);
        }
        ...
        if (its_class_name() && !vmc->no_its) {
            ...
            /* Physical Base Address */
            build_append_int_noprefix(table_data, memmap[VIRGO_GIC_ITS].base, 8);
            build_append_int_noprefix(table_data, 0, 4);    /* Reserved */
        }
    } else {
        const uint16_t spi_base = vms->irqmap[VIRGO_GIC_V2M] + ARM_SPI_BASE;
        ...
        /* Physical Base Address */
        build_append_int_noprefix(table_data, memmap[VIRGO_GIC_V2M].base, 8);
...
static void build_fadt_rev6(GArray *table_data, BIOSLinker *linker,
                            VirgoMachineState *vms, unsigned dsdt_tbl_offset)
{
    /* ACPI v6.0 */
    AcpiFadtData fadt = {
        .rev = 6,
...
/* DSDT */
static void
build_dsdt(GArray *table_data, BIOSLinker *linker, VirgoMachineState *vms)
{
    VirgoMachineClass *vmc = VIRGO_MACHINE_GET_CLASS(vms);
    Aml *scope, *dsdt;
    MachineState *ms = MACHINE(vms);
    ...
    acpi_dsdt_add_uart(scope, &memmap[VIRGO_UART],
                       (irqmap[VIRGO_UART] + ARM_SPI_BASE));
    if (vmc->acpi_expose_flash) {
        acpi_dsdt_add_flash(scope, &memmap[VIRGO_FLASH]);
    }
    acpi_dsdt_add_fw_cfg(scope, &memmap[VIRGO_FW_CFG]);
    acpi_dsdt_add_virtio(scope, &memmap[VIRGO_MMIO],
                    (irqmap[VIRGO_MMIO] + ARM_SPI_BASE), NUM_VIRTIO_TRANSPORTS);
    acpi_dsdt_add_pci(scope, memmap, irqmap[VIRGO_PCIE] + ARM_SPI_BASE, vms);
    if (vms->acpi_dev) {
        build_ged_aml(scope, "\\_SB."GED_DEVICE,
                      HOTPLUG_HANDLER(vms->acpi_dev),
                      irqmap[VIRGO_ACPI_GED] + ARM_SPI_BASE, AML_SYSTEM_MEMORY,
                      memmap[VIRGO_ACPI_GED].base);
    } else {
        acpi_dsdt_add_gpio(scope, &memmap[VIRGO_GPIO],
                           (irqmap[VIRGO_GPIO] + ARM_SPI_BASE));
    }
...
static
void virgo_acpi_build(VirgoMachineState *vms, AcpiBuildTables *tables)
{
    VirgoMachineClass *vmc = VIRGO_MACHINE_GET_CLASS(vms);
    ...
    {
        AcpiMcfgInfo mcfg = {
           .base = vms->memmap[VIRGO_ECAM_ID(vms->highmem_ecam)].base,
           .size = vms->memmap[VIRGO_ECAM_ID(vms->highmem_ecam)].size,
        };
    ...
    if (vms->iommu == VIRGO_IOMMU_VIRTIO) {
        acpi_add_table(table_offsets, tables_blob);
...
static void virgo_acpi_build_update(void *build_opaque)
{
    ...
    virgo_acpi_build(VIRGO_MACHINE(qdev_get_machine()), &tables);
...
static void virgo_acpi_build_reset(void *build_opaque)
{
    AcpiBuildState *build_state = build_opaque;
    build_state->patched = false;
}

static const VMStateDescription vmstate_virgo_acpi_build = {
    .name = "virgo_acpi_build",
    .version_id = 1,
...
void virgo_acpi_setup(VirgoMachineState *vms)
{
    ...
    if (!vms->fw_cfg) {
        // trace_virgo_acpi_setup();
        return;
    }

    if (!virgo_is_acpi_enabled(vms)) {
        // trace_virgo_acpi_setup();
        return;
    }
    ...
    acpi_build_tables_init(&tables);
    virgo_acpi_build(vms, &tables);

    /* Now expose it all to Guest */
    build_state->table_mr = acpi_add_rom_blob(virgo_acpi_build_update,
                                              build_state, tables.table_data,
                                              ACPI_BUILD_TABLE_FILE);
    assert(build_state->table_mr != NULL);

    build_state->linker_mr = acpi_add_rom_blob(virgo_acpi_build_update,
                                               build_state,
    ...
    build_state->rsdp_mr = acpi_add_rom_blob(virgo_acpi_build_update,
                                             build_state, tables.rsdp,
                                             ACPI_BUILD_RSDP_FILE);

    qemu_register_reset(virgo_acpi_build_reset, build_state);
    virgo_acpi_build_reset(build_state);
    vmstate_register(NULL, 0, &vmstate_virgo_acpi_build, build_state);
